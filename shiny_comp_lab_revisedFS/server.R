server <- function(input, output, session) {
	...
	Mutated_crit <- reactive({
		if (is.null(input$AnalCrit)) {
			...
			Mutate_VarCollapse_criteria(data = Lab_FS, collapsing_vars = input$AnalCrit)
		}
	})
	...
	# 		return(NULL)
	# 	} else if((length(input$AnalCrit) < 3) & (!is.null(input$FacetCrit))) {
	# 		# Lab_FS %>%
	# 		# 	Mutate_VarCollapse_criteria(collapsing_vars = input$AnalCrit) %>%
	# 		# 	Mutate_VarCollapse_facet(collapsing_vars = input$FacetCrit)
	# 
	# 		Mutate_VarCollapse_facet(data = Mutated_crit(),
	# 														 collapsing_vars = input$FacetCrit)
	# 	} else if((length(input$AnalCrit) > 3)){
	# 		mutate(Mutated_crit(),
	...
	Mutated <- reactive({
		if (is.null(input$AnalCrit)){
			return(NULL)
		} else if(is.null(input$FacetCrit)) {
			Lab_FS %>%
				Mutate_VarCollapse_criteria(collapsing_vars = input$AnalCrit)
		} else {
			Lab_FS %>%
	...
	plot_sizeTab1 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab1),
												 ...
												 3 * as.numeric(input$PlotSizeTab1),
	...
	plot_sizeTab2 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab2),
												 ...
												 3 * as.numeric(input$PlotSizeTab2),
	...
	output$rough_box2_UIO <- renderUI({
		if (is.null(input$FacetCrit) | is.null(input$AnalCrit)){
			...
			plotOutput("rough_box2", 
								 height = plot_sizeTab2()[[length(input$FacetCrit)]])
		}
	})
	...
	plot_sizeTab3 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab3),
												 ...
												 3 * as.numeric(input$PlotSizeTab3),
	...
	output$rough_line2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("rough_line2",
								 height = plot_sizeTab3()[[length(input$FacetCrit)]])
		}
	})
	
	
	output$rough_line2 <- renderPlot({
		if (is.null(plot_rough_line()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plot_rough_line() +
				facet_wrap(~ facet)
	...
	plotG_sizeTab3 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotGSizeTab3),
												 2 * as.numeric(input$PlotGSizeTab3),
												 3 * as.numeric(input$PlotGSizeTab3),
	...
	output$roughG_line2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | (is.null(input$FacetCrit))) {
			...
			plotOutput("roughG_line2",
								 height = plotG_sizeTab3()[[length(input$FacetCrit)]])
		}
	})
	...
	plot_sizeTabRM <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTabRM),
												 2 * as.numeric(input$PlotSizeTabRM),
												 3 * as.numeric(input$PlotSizeTabRM),
	...
	output$rough_model2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("rough_model2",
								 height = plot_sizeTabRM()[[length(input$FacetCrit)]])
		}
	})
	
	
	output$rough_model2 <- renderPlot({
		if (is.null(plot_rough_model()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plot_rough_model() +
				facet_wrap(~ facet)
	...
	# 	str_c(as.character(c(as.numeric(input$PlotGSizeTabRM),
	# 											 2 * as.numeric(input$PlotGSizeTabRM),
	# 											 3 * as.numeric(input$PlotGSizeTabRM),
	...
	# 							 height = plotG_sizeTabRM()[[length(input$FacetCrit)]])
	# 	}
	# })
	# 
	# output$roughG_model2 <- renderPlot({
	# 	if (is.null(plotG_rough_model()) | is.null(input$FacetCrit)) {
	# 		return(NULL)
	# 	} else {
	# 		plotG_rough_model() +
	# 			facet_wrap(~ facet)
	...
	plot_sizeTab4 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab4),
												 ...
												 3 * as.numeric(input$PlotSizeTab4),
	...
	output$hard_jitter2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("hard_jitter2",
								 height = plot_sizeTab4()[[length(input$FacetCrit)]])
		}
	})
	
	output$hard_jitter2 <- renderPlot({
		if (is.null(plot_hard_jitter()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
	...
	plot_sizeTab5 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab5),
												 ...
												 3 * as.numeric(input$PlotSizeTab5),
	...
	output$hard_box2_UIO <- renderUI({
		if(is.null(input$AnalCrit) | is.null(input$FacetCrit)){
			...
			plotOutput("hard_box2",
								 height = plot_sizeTab5()[[length(input$FacetCrit)]])
		}
	})
	
	output$hard_box2 <- renderPlot({
		if(is.null(plot_hard_box()) | is.null(input$FacetCrit)){
			return(NULL)
		} else {
			plot_hard_box() +
				facet_wrap(~ facet)
	...
	plot_sizeTab6 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab6),
												 ...
												 3 * as.numeric(input$PlotSizeTab6),
	...
	output$hard_line2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("hard_line2",
								 height = plot_sizeTab6()[[length(input$FacetCrit)]])
		}
	})
	
	output$hard_line2 <- renderPlot({
		if (is.null(plot_hard_line()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plot_hard_line() + 
				facet_wrap(~ facet)
	...
	plotG_sizeTab6 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotGSizeTab6),
												 2 * as.numeric(input$PlotGSizeTab6),
												 3 * as.numeric(input$PlotGSizeTab6),
	...
	plot_sizeTabHM <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTabHM),
												 2* as.numeric(input$PlotSizeTabHM),
												 3 * as.numeric(input$PlotSizeTabHM),
	...
	output$hard_model2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("hard_model2",
								 height = plot_sizeTabHM()[[length(input$FacetCrit)]])
		}
	})
	
	
	output$hard_model2 <- renderPlot({
		if (is.null(plot_hard_model()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plot_hard_model() +
				facet_wrap(~ facet)
	...
	# 	str_c(as.character(c(as.numeric(input$PlotGSizeTabHM),
	# 											 2 * as.numeric(input$PlotGSizeTabHM),
	# 											 3 * as.numeric(input$PlotGSizeTabHM),
	...
	# 							 height = plotG_sizeTabHM()[[length(input$FacetCrit)]])
	# 	}
	# })
	# 
	# output$hardG_model2 <- renderPlot({
	# 	if (is.null(plotG_hard_model()) | is.null(input$FacetCrit)) {
	# 		return(NULL)
	# 	} else {
	# 		plotG_hard_model() +
	# 			facet_wrap(~ facet)
	...
	plot_sizeTab7 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab7),
												 ...
												 3 * as.numeric(input$PlotSizeTab7),
	...
	output$flexural_jitter2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)){
			...
			plotOutput("flexural_jitter2",
								 height = plot_sizeTab7()[[length(input$FacetCrit)]])
		}
	})
	
	
	output$flexural_jitter2 <- renderPlot({
		if (is.null(plot_flexural_jitter()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else{
			plot_flexural_jitter() +
				facet_wrap(~ facet)
	...
	plot_sizeTab8 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab8),
												 ...
												 3 * as.numeric(input$PlotSizeTab8),
	...
	output$flexural_box2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)){
			...
			plotOutput("flexural_box2",
								 height = plot_sizeTab8()[[length(input$FacetCrit)]])
		}
	})
	
	output$flexural_box2 <- renderPlot({
		if (is.null(plot_flexural_box()) | is.null(input$FacetCrit)){
			return(NULL)
		} else {
			plot_flexural_box() +
				facet_wrap(~ facet)
	...
	plot_sizeTab9 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTab9),
												 ...
												 3 * as.numeric(input$PlotSizeTab9),
	...
	output$flexural_line2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("flexural_line2",
								 height = plot_sizeTab9()[[length(input$FacetCrit)]])
		}
	})
	
	output$flexural_line2 <- renderPlot({
		if (is.null(plot_flexural_line()) | is.null(input$FacetCrit)){
			return(NULL)
		} else {
			plot_flexural_line() +
				facet_wrap(~ facet)
	...
	plotG_sizeTab9 <- reactive({
		str_c(as.character(c(as.numeric(input$PlotGSizeTab9),
												 2 * as.numeric(input$PlotGSizeTab9),
												 3 * as.numeric(input$PlotGSizeTab9),
	...
	output$flexuralG_line2_UIO <- renderUI({
		if (is.null(input$AnalCrit) | is.null(input$FacetCrit)) {
			...
			plotOutput("flexuralG_line2",
								 height = plotG_sizeTab9()[[length(input$FacetCrit)]])
		}
	})
	
	output$flexuralG_line2 <- renderPlot({
		if (is.null(plotG_flexural_line()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plotG_flexural_line() +
				facet_wrap(~ facet)
	...
	plot_sizeTabFM <- reactive({
		str_c(as.character(c(as.numeric(input$PlotSizeTabFM),
												 2 * as.numeric(input$PlotSizeTabFM),
												 3 * as.numeric(input$PlotSizeTabFM),
		...
		if (is.null(Mutated())) {
			...
			Mutated() %>%
				...
				ggplot(aes(x = Time,
									 y = Max_Flexural_Stress,
									 colour = criteria)) +
				scale_x_continuous("Time", breaks = c(0,30,90)) +
				stat_summary(fun.y = mean, geom = "point", size = 3, na.rm = TRUE) +
				geom_smooth(lwd = 1, se = FALSE) +
				scale_colour_manual (values = 
														 	unique(filter(colour, material != "R") [[colour_scheme()]])) +
	...
	output$flexural_model2_UIO <- renderUI({
		if (is.null(input$AnalCrit)| is.null(input$FacetCrit)) {
			...
			plotOutput("flexural_model2",
								 height = plot_sizeTabFM()[[length(input$FacetCrit)]])
		}
	})
	
	
	output$flexural_model2 <- renderPlot({
		if (is.null(plot_flexural_model()) | is.null(input$FacetCrit)) {
			return(NULL)
		} else {
			plot_flexural_model() +
				facet_wrap(~ facet)
